 ...
 */
export const MIN_USEFUL_BYTES = 4096;

/**
 * הרוחב המרבי של תמונת הפתיחה, והמסנן שגוזר אותה.
 *
 * 1080, אותו רוחב כמו הסרטון המוגש מאז שהקטלוג שודרג ב-20 באוגוסט 2026.
 * הפוסטר הוא לא רק מה שמוצג עד הלחיצה על נגן: בספריית הסרטונים ובמסך
 * החימום הוא התמונה שרואים כל הזמן, ופוסטר רך יותר מהסרטון שמאחוריו
 * בולט שם מיד.
 *
 * ההיסטוריה: עד השדרוג הרוחב כאן היה 540, כשהסרטון עצמו הוגש ב-720.
 * הנימוק אז — התמונה נטענת בכל כניסה לתרגיל ונעלמת ברגע שמנגנים, וב-720
 * הקבצים יצאו 170KB עד 230KB, מה שהורגש כהמתנה ברשת סלולרית. כשהסרטונים
 * עלו ל-1080 השיקול התהפך: החדות של תמונה שנשארת על המסך גברה על המשקל.
 *
 * מסנן thumbnail בוחר פריים מייצג מתוך חלון ולא את הראשון שנקרה בדרך,
 * ו-min(...,iw) לא מותח קליפ שצר מהתקרה. הקבועים מיוצאים כי גם
 * scripts/posters-refresh.ts גוזר איתם פוסטרים, ומספר שיושב בשני מקומות
 * כבר השאיר פעם את אחד מהם מאחור.
 */
export const POSTER_MAX_WIDTH = 1080;
export const POSTER_SCALE_FILTER = `thumbnail,scale='min(${POSTER_MAX_WIDTH},iw)':-2`;

/**
 * 5 ולא 3. נמדד: ההפרש בין השניים הוא כרבע מהמשקל, ובגודל שבו התמונה
 * מוצגת בטלפון הוא לא נראה.
 */
export const POSTER_JPEG_QUALITY = "5";

/**
 * תקרת גודל לקובץ שממנו מחלצים פריים.
 *
 * נמוכה בכוונה מהתקרה של הדחיסה. הדחיסה רצה לבדה בהפעלה משלה, וכאן
 * מדובר בעבודה שרצה על קליפ שכבר נדחס. קובץ גדול מזה מסמן שמשהו לא
 * כשורה, ועדיף לדלג עליו מאשר לשרוף את תקרת הזמן של הפונקציה.
 *
 * המרווח נבדק ב-18 באוגוסט 2026 אחרי המעבר ל-1080: הכבד בקטלוג הוא
 * 16.4MB, כלומר עשירית מהתקרה.
 */
 ...
 */
export const SEEK_FRACTIONS = [0.4, 0.6, 0.25, 0.75];
  ...
  return new Promise<void>((resolve, reject) => {
    const child = spawn(
      ...
      [
        ...
        "-frames:v", "1",
        // הרוחב והנימוק לו — כולל למה הוא היה 540 פעם — בהערה שמעל
        // POSTER_SCALE_FILTER למעלה.
