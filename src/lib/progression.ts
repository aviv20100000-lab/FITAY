...
export async function evaluateProgression(options: {
  ...
  for (const item of items) {
    ...
    if (atFinalStanceLevel && cleanCeiling) {
       ...
       *
       * מרגע שהמילוי נעצר בתקרה (13 באוגוסט 2026) אי אפשר יותר להגדיל את
       * הסך, ולכן בדיקת התקיעות הפכה למבחן שאי אפשר לעבור בו: כל אימון
       * שאינו מושלם נספר "בלי שיפור", ואחרי שניים המתאמן נשלח לתחתית
       * הטווח. איתי אמר במפורש שהמתאמן נשאר בתקרה עד שהוא עובר שלב, ולכן
       * כאן המנגנון שותק ומשאיר אותו במקומו.
       *
       * ceilingStreak הוא הסימן שהתקרה כבר הושגה. בצירים האלה אין לו
       * שימוש אחר, כי מסלול ההקשיה שייך לתרגילי מנח בלבד.
       *
       * הסימן דורש שהמתאמן עדיין נוגע בתקרה באחד הסטים. מי שירד ממנה
       * לגמרי חוזר לבדיקת התקיעות הרגילה, אחרת ביקור אחד בתקרה היה
       * משתיק את המנגנון לתמיד גם למי שנחלש באמת.
       */
      next = {
