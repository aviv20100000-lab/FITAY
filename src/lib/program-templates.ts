/**
 * תבניות התוכניות — שלוש רמות, כל אחת בשני שלבים של 4 שבועות.
 *
 ...
 * הם נקודת פתיחה סבירה שאיתי אמור לכוונן. החוברת מגדירה שיטת התקדמות,
 * לא טבלת מספרים.
 *
 * כל תבנית נזרעת כ-is_template=1. איתי משכפל ממנה תוכנית אישית למתאמן,
 * והמקור נשאר נקי.
 */
...
export const TEMPLATES: ProgramTemplate[] = [
  // ───────────────────────────────────────────────────────────── רמה 1
  {
    ...
    description:
      "נקודת הכניסה. טבעות גבוהות וגוף זקוף — הקושי מגיע מהטכניקה ולא מהמשקל. המטרה בשלב הזה היא ללמוד את ארבעת החוקים ואת הקצב, לא להתיש.",
    level: 1,
    weeks: 8,
    phases: [
      {
